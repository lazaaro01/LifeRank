  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <Panel className="p-6 md:p-10">
        ...
        <h2 className="mt-2 text-xl md:text-2xl font-bold text-accent-slate">Transforme rotina em pontos</h2>
        ...
        <form onSubmit={handleSubmit} className="mt-8 grid gap-6">
            ...
            {(Object.entries(activityCatalog) as [ActivityType, (typeof activityCatalog)[ActivityType]][]).map(
              ([key, item]) => (
                ...
                >
                  ...
                  <p className="mt-1 md:mt-2 text-xs md:text-sm font-bold text-accent-slate leading-tight">{item.label}</p>
            ...
            {type === "study" ? (
              <label className="grid gap-2 text-xs font-bold uppercase tracking-widest text-slate-400">
                ...
                <input
                  ...
                  className="rounded-2xl border border-slate-100 bg-slate-50/50 px-4 py-3 outline-none transition-all focus:border-brand-400 focus:bg-white"
                />
              </label>
            ) : null}
          ...
          <div className="flex items-center justify-between rounded-3xl bg-brand-50/50 p-6">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-brand-600">Pontuação prevista</p>
              <p className="mt-1 text-3xl font-black text-accent-slate">{currentPoints} <span className="text-sm font-bold text-slate-400">pts</span></p>
            </div>
          ...
          <button 
            type="submit" 
            className="rounded-[24px] bg-brand-600 py-5 font-bold text-white shadow-lg shadow-brand-500/20 transition-all hover:bg-brand-700 active:scale-95"
          >
            Salvar atividade
          </button>
        </form>
      ...
      <Panel className="p-6 md:p-10">
        ...
        <h2 className="mt-2 text-xl md:text-2xl font-bold text-accent-slate">Últimas atividades</h2>
        ...
        <div className="mt-8 grid gap-4">
          {myActivities.length === 0 ? (
            ...
            myActivities.map((activity) => (
              <div key={activity.id} className="rounded-[28px] border border-slate-50 bg-white p-5 transition-all hover:shadow-xl hover:shadow-brand-500/5">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="h-12 w-12 rounded-xl bg-slate-50 flex items-center justify-center text-xl shadow-inner">
                      {activityCatalog[activity.type]?.emoji || "⚡"}
                    </div>
                    <div>
                      <p className="font-bold text-accent-slate leading-tight">{activity.label}</p>
                      <p className="mt-1 text-xs text-slate-400 font-medium lowercase">
                        {activity.date} {activity.hours ? `• ${activity.hours}h` : ""}
                      </p>
                    </div>
                  </div>
                  <p className="rounded-xl bg-brand-500 px-4 py-2 text-sm font-black text-white shadow-md shadow-brand-500/10">
                    +{activity.points}
                  </p>
                </div>
              </div>
